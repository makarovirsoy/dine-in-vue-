      ...
      <div class="ml-20 py-4 flex-grow flex flex-col">
        <div
          ...
          class="
            ...
            focus:ring-4 focus:outline-none focus:ring-fuchsia-600
            font-medium
            rounded-full
            text-base
            px-6
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg mx-20">
          <table class="w-full text-sm text-left text-gray-500">
            <thead class="text-xs text-fuchsia-500 uppercase bg-fuchsia-100">
              <tr>
                <th scope="col" class="px-6 py-3">Kategorien</th>
                <th scope="col" class="px-6 py-3">Position</th>
                <th scope="col" class="px-6 py-3">id</th>
                <th scope="col" class="px-6 py-3">Bearbeiten</th>
            ...
            <tbody>
              <tr
                ...
                v-for="category in this.$data.categories"
              >
                <th scope="row" class="px-6 py-4 font-medium text-fuchsia-900">
                  {{ category.name }}
                </th>
                <th scope="row" class="px-6 py-4 font-medium text-fuchsia-900">
                  <button @click="toTop(category.id)">Top</button>
                  {{ " " }}
                  <button @click="toBottom(category.id)">Down</button>
                </th>
                ...
                <td class="px-6 py-4 text-left">
                  <div
                    ...
                    class="font-medium text-red-600 hover:cursor-pointer"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      class="h-6 w-6"
...
export default {
  ...
  methods: {
    deleteCategory(id) {
      ...
      axios
        .delete(url_api + "api/categories/" + id, {
    ...
    toBottom(id) {
      axios
        .put(
          url_api + "api/categories/bottom/" + id,
          {},
          {
            headers: { Authorization: "Bearer " + this.$cookies.get("token") },
          }
